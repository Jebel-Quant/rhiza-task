...
PRE_COMMIT_SHIM = "File generated by pre-commit"
"""The header classic pre-commit writes into the hook it installs.

Read rather than merely noticing that *a* hook is there, because the three things that hook
could be want three different answers and only its first lines tell them apart: prek writes
``File generated by prek`` into its own shim, and a hand-written hook carries neither
marker. Verified against pre-commit 4.4.0 and prek 0.5.2 rather than assumed.
"""


def _generated_by_pre_commit(path: Path) -> bool:
    """Say whether a path holds a hook classic pre-commit generated.

    Args:
        path: The hook, which need not exist.

    Returns:
        True when the file is readable and carries pre-commit's own header. A missing,
        unreadable or binary file is not one, which is why the read is guarded rather than
        preceded by an ``is_file`` check.
    """
    try:
        return PRE_COMMIT_SHIM in path.read_text(errors="replace")
    except OSError:
        return False


def _hook_path(root: Path, git: str) -> Path:
    """Locate the ``pre-commit`` hook, asking git where this repository keeps its hooks.

    ``root / ".git" / "hooks"`` is wrong wherever ``.git`` is a *file* rather than a
    directory: a linked worktree and a submodule both keep their hooks under the parent's
    git directory. A check that looked in the wrong place would report "no legacy hook" for
    exactly the checkouts most likely to be carrying one, and silently.
    ...
    Returns:
        Where the hook would be, whether or not it exists. Falls back to the default
        location when git answers nothing, which is what happens outside a repository --
        where nothing exists to be found and the question does not matter.
    """
    named = _git(git, ["rev-parse", "--git-path", "hooks/pre-commit"], root, capture=True).strip()
    return Path(named) if Path(named).is_absolute() else root / (named or ".git/hooks/pre-commit")
...
def install_hooks(cfg: Config) -> None:
    ...
    **``--force`` is passed when, and only when, the hook in place is pre-commit's own.**
    Bare ``prek install`` on a repository that already has a classic hook moves it to
    ``pre-commit.legacy`` and enters *migration mode*, where prek runs the legacy hook too
    -- and pre-commit's generated script refuses to run in that situation, so ``git commit``
    fails, telling the reader to report a bug against pre-commit. It does not heal: once the
    ``.legacy`` file exists every later ``install`` re-enters migration mode, and ``install``
    is a prerequisite of nearly every gate, so the broken commit path is reaffirmed by each
    gate the consumer runs. This lands on the 0.x upgrade, which is to say on everybody. See
    #171.

    So the condition is the *file*, not the flag. Forcing unconditionally would be simpler
    and would take a hand-written ``.git/hooks/pre-commit`` with it, on the next ``test``,
    for one line of output -- and ``core.hooksPath`` does not cover a hook someone wrote in
    the default location. Migration mode is the right answer for that hook: it keeps
    running, prek says it moved it, and the ``.legacy`` it leaves is not a generated shim, so
    a later run reaches this check and still keeps hands off. What is discarded here is only
    ever a script the tool that wrote it says to remove.
    ...
    if hooks_path:
        ...
        return
    hook = _hook_path(cfg.root, git)
    # Both, because they are two stages of one state: the live hook before prek has moved
    # it, the `.legacy` sibling after. Either one being pre-commit's means the commit path
    # is broken or about to be.
    legacy = hook.parent / f"{hook.name}.legacy"
    stale = [path for path in (hook, legacy) if _generated_by_pre_commit(path)]
    if stale:
        print(f"[INFO] replacing pre-commit's own hook ({', '.join(path.name for path in stale)}) with prek's")
    ...
    uvx("prek", "install", *force, "-c", ".pre-commit-config.yaml", cwd=cfg.root, check=False)
