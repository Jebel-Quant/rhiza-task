...
```bash
uvx rhiza-task@1.8.0 list          # what is available
uvx rhiza-task@1.8.0 all           # every gate, as CI runs them
uvx rhiza-task@1.8.0 test --strict # fail rather than skip when a gate measures nothing
```

!!! tip "Pin the version"
    `rhiza-task@1.8.0` rather than `rhiza-task`. The pin is the whole point of the
...
```bash
uvx rhiza-task@1.8.0 test          # shorthand
uvx rhiza-task@1.8.0 run test      # the same thing
uvx rhiza-task@1.8.0 run fmt test  # several, in order, prerequisites deduplicated
```
...
```bash
uvx rhiza-task@1.8.0 all --strict
```
...
- name: Gates
  run: uvx rhiza-task@1.8.0 all --strict
```

```bash
uvx rhiza-task@1.8.0 ci-os-matrix
```
