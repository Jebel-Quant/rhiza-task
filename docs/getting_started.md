...
!!! tip "Pin the version"
    `rhiza-task@1.4.1` rather than `rhiza-task`. The pin is the whole point of the
...
```bash
uvx rhiza-task@1.4.1 test          # shorthand
uvx rhiza-task@1.4.1 run test      # the same thing
uvx rhiza-task@1.4.1 run fmt test  # several, in order, prerequisites deduplicated
```
...
- name: Gates
  run: uvx rhiza-task@1.4.1 all --strict
```

```bash
uvx rhiza-task@1.4.1 ci-os-matrix
```
