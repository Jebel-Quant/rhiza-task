...
```bash
uvx rhiza-task@1.3.1 list          # what is available
uvx rhiza-task@1.3.1 all           # every gate, as CI runs them
uvx rhiza-task@1.3.1 test --strict # fail rather than skip when a gate measures nothing
```

!!! tip "Pin the version"
    `rhiza-task@1.3.1` rather than `rhiza-task`. The pin is the whole point of the
...
```bash
uvx rhiza-task@1.3.1 test          # shorthand
uvx rhiza-task@1.3.1 run test      # the same thing
uvx rhiza-task@1.3.1 run fmt test  # several, in order, prerequisites deduplicated
```
...
- name: Gates
  run: uvx rhiza-task@1.3.1 all --strict
```

```bash
uvx rhiza-task@1.3.1 ci-os-matrix
```
