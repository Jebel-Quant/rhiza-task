...
`rhiza-task@1.3.0` makes a bump a deliberate commit that carries whatever fixes it wants.
...
```bash
uvx rhiza-task@1.3.0 all --root ../other-repo
```
...
```bash
RHIZA_CI_OS_MATRIX= uvx rhiza-task@1.3.0 ci-os-matrix   # still ["ubuntu-latest"]
```
