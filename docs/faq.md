...
`rhiza-task@1.4.1` makes a bump a deliberate commit that carries whatever fixes it wants.
...
```bash
uvx rhiza-task@1.4.1 all --root ../other-repo
```
...
```bash
RHIZA_CI_OS_MATRIX= uvx rhiza-task@1.4.1 ci-os-matrix   # still ["ubuntu-latest"]
```
